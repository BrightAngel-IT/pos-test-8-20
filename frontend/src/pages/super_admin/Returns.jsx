...
export default function Returns({ api, session, onNotice, refreshCoreData }) {
  ...
  async function fetchBranches() {
    try {
      const response = await api.get('/branches', authConfig(session.token))
      ...
      if (newReturn.type === 'customer') {
        ...
        if (relevantSales.length > 0) {
          ...
          setNewReturn(prev => ({
            ...
            branch: latestSale.branch,
            items: latestSale.items.map(item => {
              const returnedQty = returnedQuantities[item.productId] || 0;
              const maxQty = Math.max(0, item.quantity - returnedQty);
              // Only auto-select 1 if there's enough maxQuantity left
        ...
        <div className="grid-12 gap-4 align-center wrap-row">
          ...
          <div className="grid-colspan-3 stack gap-1">
            ...
            >
              ...
              <option value="entityName">👤 {returnMode === 'customer' ? 'Customer Name' : 'Supplier Name'}</option>
