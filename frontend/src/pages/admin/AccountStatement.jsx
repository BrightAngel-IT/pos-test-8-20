  ...
  async function fetchData() {
    ...
    try {
      ...
      const [invoicesRes, paymentsRes, returnsRes, salesRes] = await Promise.all([
        ...
        type === 'customer' ? api.get(`/sales?customerId=${id}`, config).catch(() => ({ data: { sales: [] } })) : Promise.resolve({ data: { sales: [] } })
      ])
  ...
  const statementData = useMemo(() => {
    const transactions = [
      ...invoices.map(inv => ({
        ...
        date: inv.date,
        type: 'Invoice',
        reference: (inv.invoiceNo || '').replace(/^saayi-?c?-?/i, 'INVC-'),
      ...
      ...immediatePayments.flatMap(p => [
        {
          ...
          reference: (p.reference || '').replace(/^saayi-?c?-?/i, 'INVC-'),
          method: 'BILLING',
          billing: p.amount,
          payment: 0,
          status: 'PAID',
