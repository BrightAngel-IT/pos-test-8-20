...
export const printSettlementReceipt = (payment, user, company = null) => {
  ...
  const totalAllocated = (payment.allocations || []).reduce((sum, a) => sum + Number(a.allocatedAmount || 0), 0)
  const receivedAmount = Number(payment.totalAmount || 0)
  const unallocatedBalance = Math.max(0, receivedAmount - totalAllocated)

  const rows = (payment.allocations || [])
    .map(
      (alloc, index) => {
        const invNo = alloc.invoiceId?.invoiceNo || alloc.invoiceId || 'N/A'
        ...
        <div style="display: flex; margin-bottom: 6px; font-size: 11px; line-height: 1.2;">
          <div style="width: 20px; text-align: center; font-weight: 700;">${index + 1}</div>
          <div style="flex: 1;">
            <div style="display: flex; justify-content: space-between; font-weight: 700; font-family: 'Courier New', Courier, monospace;">
              <span style="flex: 1; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">${invNo}</span>
              <span style="width: 80px; text-align: right; font-weight: 900;">${amount.toFixed(2)}</span>
            </div>
          </div>
        </div>
      `
  ...
  receiptWindow.document.write(`
    <html>
      <head>
        <title>Settlement Receipt - ${payment.paymentNo || `PAY-${payment.localId}`}</title>
        <style>
          @media print {
            @page { margin: 0; }
            body { margin: 0; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
      ...
      <body style="font-family: Arial, Helvetica, sans-serif; width: 300px; margin: 0 auto; color: #000; line-height: 1.2; font-smooth: never; -webkit-font-smoothing: none; position: relative;">
        ...
        <div style="text-align: center; margin-bottom: 6px; padding-top: 0; margin-top: 2px;">
          ...
          <div style="font-size: 16px; font-weight: 900; margin-top: 10px; text-decoration: underline;">SETTLEMENT RECEIPT</div>
        </div>

        <div style="font-size: 11px; margin-bottom: 6px; font-weight: 700;">
          <div style="display: flex; justify-content: space-between;">
            <span>DATE: ${new Date(payment.createdAt || payment.paymentDate || Date.now()).toLocaleDateString()}</span>
            <span>TIME: ${new Date(payment.createdAt || payment.paymentDate || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
          </div>
          <div style="display: flex; justify-content: space-between; margin-top: 2px;">
            <span>REF: ${payment.paymentNo || `PAY-${payment.localId}`}</span>
            <span>CASHIER: ${user?.name ? user.name.toUpperCase() : 'N/A'}</span>
          </div>
          <div style="margin-top: 2px; border-bottom: 1px solid #000; padding-bottom: 2px;">CUSTOMER: ${payment.customerName ? payment.customerName.toUpperCase() : (payment.supplierName ? payment.supplierName.toUpperCase() : 'N/A')}</div>
        </div>
        ...
        <div style="margin-bottom: 10px;">
          ${rows.length > 0 ? rows : '<div style="text-align: center; font-size: 11px; margin: 10px 0;">No invoices allocated</div>'}
        </div>
        ...
        <div style="font-size: 13px; margin-left: 10%; font-weight: 700;">
          ...
          <div style="display: flex; justify-content: space-between; margin-bottom: 1px; color: #444;">
            <span>METHOD:</span>
          ...
          <div style="display: flex; justify-content: space-between; font-weight: 700; font-size: 14px; margin-top: 5px;">
            ...
            <span>${unallocatedBalance.toFixed(2)}</span>
          </div>
        </div>
        ...
        <div style="text-align: center; font-weight: 700;">
          <div style="font-size: 10px; margin-bottom: 6px;">INVOICES SETTLED: ${(payment.allocations || []).length}</div>
          <div style="font-weight: 900; margin: 6px 0; font-size: 13px;">*** THANK YOU ***</div>
